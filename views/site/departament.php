        ...
        <div class="dean d-flex justify-content-center align-items-center col-md-4">
            ...
            <div class="information-dean">
                ...
                <div class="d-flex justify-content-center gap-3 my-2">
                    <a href="mail:to:<?= !empty($dean['email']) ? $dean['email'] : Yii::t('app', 'not specified') ?>">
                    ...
                    <?= Html::a(Yii::t('app', 'To view the history of the department, follow the link'), ['site/history-departament', 'departament_id' => $departament_id], ['class' => 'history-departament']) ?>
                </div>
        ...
        <div class="teachers-container">
            <?php foreach ($teachers as $teacher): ?>
                ...
                <div class="teachers-box" onclick="openTeachersBox(this)" data-fio="<?= Html::encode($fio) ?>"
                    ...
                    data-email="<?= Html::encode(strtolower($teacher['email'])) ?>"
                    data-workexperience="<?= Html::encode($teacher['work_experience']) ?>"
                    data-academicdegree="<?= Html::encode($teacher['academic_degree']) ?>"
                    data-academicrank="<?= Html::encode($teacher['academic_rank']) ?>"
                    data-text-fullname="<?= Yii::t('app', 'Full Name') ?>"
                    data-text-job-title="<?= Yii::t('app', 'Job Title') ?>"
                    data-text-work-experience="<?= Yii::t('app', 'Work Experience') ?>"
                    data-text-academic-degree="<?= Yii::t('app', 'Academic Degree') ?>"
                    data-text-academic-rank="<?= Yii::t('app', 'Academic Rank') ?>"
                    data-text-year="<?= Yii::t('app', 'year') ?>" 
                    data-text-email="<?= Yii::t('app', 'Email') ?>"
