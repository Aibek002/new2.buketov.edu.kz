document.addEventListener("DOMContentLoaded", function () {
  ...
  select_board_subsec.addEventListener("change", function () {
    ...
    if (
      ...
      selectedYear(select_year, select_file, name_url, submitButton, remove);
    }
  });
  ...
  committee_subsection.addEventListener("change", function () {
    resetCommitteeSubsectionFields(select_year, select_file, name_url);
    if (committee_subsection.value !== "Положение") {
      selectedYear(select_year, select_file, name_url, submitButton, add);
      selectFile(select_file, name_url, submitButton, remove);
    } else {
      selectedYear(select_year, select_file, name_url, submitButton, remove);
      selectFile(select_file, name_url, submitButton, add);
    }
  });

  // 🔄 Обработка смены категории
  select_subsection.addEventListener("change", function () {
    resetAllFields(select_board_subsec, select_year, select_file, name_url);
    submitButton.classList.remove("active");
...
function selectedYear(year, select_file, name_url, submitButton, type) {
  if (type === "add") {
    year.classList.add("active");
    year.addEventListener("change", function () {
      if (year.selectedIndex > 0) {
        selectFile(select_file, name_url, submitButton, type);
      }
    });
  } else {
    selectFile(select_file, name_url, submitButton, type);
...
function selectFile(select_file, name_url, submitButton, type) {
  if (type === "add") {
...
function resetCommitteeSubsectionFields(select_year, select_file, name_url) {
